...
// biding queue
// connect blob
// connect google vision and get info from img
// connect to db
async function main() {
    const blobServiceClient = BlobServiceClient.fromConnectionString(connectionString);
    const containerClient = blobServiceClient.getContainerClient(blobContainerName);
    const blockBlobClient = containerClient.getBlockBlobClient("te.png");
    ...
    try {
        ...
        console.log(`Product info inserted with _id: ${result.insertedId}`);

        // Close the MongoDB connection
        await mongoClient.close();
        console.log('Inserted product data into the database');
    } catch (error) {
        console.log('Error inserting product data into the database:', error);
    } finally {
        await mongoClient.close();
    }
}
